#!/usr/bin/env bash
# Parallel, resumable Apollo 11 localization pipeline.
#
...
if (( $# > 0 )); then
    case "$1" in
        -h|--help)
            echo "Usage: ./run_apollo11_parallel_pipeline.sh"
...
mkdir -p "$LOG_DIR"
exec > >(tee -a "$LOG_DIR/haworth_parallel_pipeline.log") 2>&1
trap 'status=$?; echo "FAILED (exit $status) at line $LINENO. Log: $LOG_DIR/haworth_parallel_pipeline.log"; exit $status' ERR
...
echo "Apollo 11 parallel localization pipeline"
...
echo "Log:             $LOG_DIR/haworth_parallel_pipeline.log"

run "$PYTHON" -c \
    "import sys; sys.path.insert(0, '$PROJECT'); from pipeline_config import load_pipeline_config; c=load_pipeline_config(); assert c.config_path.name == 'apollo11.yaml'; assert c.truth_source_profile == '2m'; print('Config:', c.config_path); print('Truth:', c.truth_source_label)"
...
echo
echo "Apollo 11 parallel pipeline completed."
