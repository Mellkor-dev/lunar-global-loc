...
`config/haworth.yaml` is the authority for every input and generated artifact
used by the Haworth experiment. Scripts must load paths through
`pipeline_config.load_pipeline_config()` rather than reconstructing them.
The configured 1 m/px profile is the truth source for DEM validation and
...
`feature_detection.resolutions` in `config/haworth.yaml`. Run one native
...
316 synchronized OS1 scan/odometry/transform triplets stored under
`sim/0p25m_px`.
